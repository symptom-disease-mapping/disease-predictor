...
class AuthUser(AbstractUser):
    ...
    class Meta:
        managed = False
        db_table = 'auth_user'
    # Specify custom intermediate table names to resolve clashes
    groups = models.ManyToManyField(
        'auth.Group',
        verbose_name='groups',
        related_name='auth_user_groups_custom',  # Unique related_name
        blank=True,
        help_text='The groups this user belongs to. A user will get all permissions granted to each of their groups.',
        db_table='auth_user_groups_custom',  # Custom intermediate table name
    )
    user_permissions = models.ManyToManyField(
        'auth.Permission',
        verbose_name='user permissions',
        related_name='auth_user_permissions_custom',  # Unique related_name
        blank=True,
        help_text='Specific permissions for this user.',
        db_table='auth_user_permissions_custom',  # Custom intermediate table name
    )
    # Change is_superuser to BooleanField
    is_superuser = models.BooleanField(default=False)  # Assuming default is False, adjust as needed
...
class PostsInfo(models.Model):
    username = models.CharField(max_length=20, primary_key=False, default='default_username')
    age=models.IntegerField()
    gender = models.CharField(max_length=20)
    disease = models.CharField(max_length=255, blank=True)  # Add a new field for storing diseases
